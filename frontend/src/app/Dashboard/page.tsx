  ...
  const generatePlan = async (formData: any) => {
    try {
      ...
      if (!response.ok || !data.success) {
        setError(data?.message || "Backend returned an error. Please try again.");
        return;
      }
      ...
      setError("Backend temporarily unavailable. Please try again in a few seconds.");
    } finally {
      setLoading(false);
    }
  };
...
function ProfessionalResultSection({ result }: { result: any }) {
  ...
  const safeMealDays = (result?.meal_plan?.days ?? []).map((day: any) => {
    ...
    return {
      ...
      workout_tip: day?.workout_tip?.trim() || generateWorkoutTip(goal, activity),
        ...
        <div className="relative z-10 mb-10 flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <p className="mt-5 max-w-2xl text-base leading-8 text-zinc-400">
              Generated using your body profile, goal, activity level, and AI
        ...
        <div className="w-full max-w-4xl rounded-[40px] border border-green-500/15 bg-zinc-950/90 p-8 text-center shadow-[0_0_60px_rgba(34,197,94,0.1)]">
          <div className="mb-6 text-center">
            ...
            <p className="mt-4 text-zinc-400">
              ...
              profile, body analytics, and meal goals.
            </p>
          </div>
    ...
    <section className="mt-12">
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4">
        ...
        <FeatureCard
          ...
          description="Track BMI, BMR, TDEE, macros, hydration, and progress signals."
        />
